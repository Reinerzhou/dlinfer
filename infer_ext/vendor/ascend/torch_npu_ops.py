...
def context_attention(
    ...
    q_hidden_dim = num_q_heads * query.size(-1)
    kv_hidden_dim = num_kv_heads * query.size(-1)
    for i in range(batch):
        start = q_start_loc[i]
        end = start + seq_len[i]
        single_q = query[start:end].view(1, -1, q_hidden_dim)
        single_k = key[start:end].reshape(1, -1, kv_hidden_dim)
        single_v = value[start:end].reshape(1, -1, kv_hidden_dim)
        single_o = attn_output[start:end].view(1, -1, q_hidden_dim)
